 ...
 * และส่งรูปภาพ (Vision) แบบ base64
 */
public class GeminiAssistant {
    ...
    /** โมเดลที่รองรับรูป (Groq Vision) — เปลี่ยนได้ใน AI Settings ภายหลัง */
    ...
    private String getVisionModel() {
        ...
        String model = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                .getString(KEY_VISION_MODEL, DEFAULT_VISION_MODEL);
        if (model == null || model.trim().isEmpty()) return DEFAULT_VISION_MODEL;
        return model.trim();
    }
    ...
    /**
     * ถาม AI พร้อมรูป (base64 ไม่มี prefix data:...)
     * @param prompt      คำถาม
     * @param imageBase64 ข้อมูลรูป base64 (null = text อย่างเดียว)
     * @param mimeType    เช่น "image/jpeg", "image/png" (default jpeg)
     * @param history     ประวัติ (text อย่างเดียว — รอบที่มีรูปไม่ยัดลง history ยาว)
     * @param temperature 0–1
     * @param maxTokens   จำกัดคำตอบ
     * @param callback    ผลลัพธ์
     */
    public void askAI(final String prompt,
        ...
        // ไม่มีรูป → ใช้ path เดิม
        if (imageBase64 == null || imageBase64.trim().isEmpty()) {
            askAI(prompt, history, temperature, maxTokens, callback);
            return;
        }
        ...
        new Thread(() -> {
            ...
            try {
                ...
                conn.setDoOutput(true);
                conn.setConnectTimeout(30000);
                conn.setReadTimeout(120000); // รูปรอได้นานขึ้น

                JSONObject body = new JSONObject();
                body.put("model", getVisionModel());
                body.put("temperature", Math.max(0.0, Math.min(1.0, temperature)));
                body.put("max_tokens", Math.max(64, Math.min(8192, maxTokens)));
                ...
                // User + รูป (multimodal content array)
                String mime = (mimeType != null && mimeType.contains("/"))
                        ? mimeType.trim()
                        : "image/jpeg";
                String dataUrl = "data:" + mime + ";base64," + imageBase64.trim();

                JSONArray contentArr = new JSONArray();

                JSONObject textPart = new JSONObject();
                textPart.put("type", "text");
                textPart.put("text", prompt != null ? prompt : "อธิบายภาพนี้");
                ...
                callback.onError("หมดเวลาการเชื่อมต่อ (timeout) — รูปใหญ่หรือเน็ตช้า ลองย่อรูปแล้วส่งใหม่");
            } catch (java.net.UnknownHostException e) {
                callback.onError("ไม่พบเซิร์ฟเวอร์ — ตรวจสอบการเชื่อมต่ออินเทอร์เน็ต");
            } catch (Exception e) {
                String msg = e.getMessage();
    ...
    /** เวอร์ชันสั้น: มีรูป + callback */
    public void askAI(final String prompt, final String imageBase64,
                      final String mimeType, final AICallback callback) {
        askAI(prompt, imageBase64, mimeType, null, 0.4, 2048, callback);
    }
